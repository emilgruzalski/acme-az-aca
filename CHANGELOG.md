...
- Dependabot updates for Go modules, GitHub Actions, and the Docker base images.
- CI security gates: `govulncheck` and a gating Trivy image scan (HIGH/CRITICAL) on every run; release images get an SPDX SBOM, BuildKit SBOM/provenance attestations, and a keyless cosign signature. Trivy results are also uploaded as SARIF to the GitHub Security tab.
...
- CI is a repo-local workflow tailored to this app (gofmt and `go mod tidy` checks, vet, golangci-lint v2, race tests, Docker build with image push to ghcr.io on `v*` tags) instead of the shared reusable workflow; it also runs on pushes to `main`.
