...
- Bumped `golang.org/x/crypto` from `v0.53.0` to `v0.55.0` for [CVE-2026-56854](https://avd.aquasec.com/nvd/cve-2026-56854) (CRITICAL; auth bypass from unenforced source-address restrictions in `x/crypto/ssh`). This code never calls `ssh`, so `govulncheck` treats it as unreachable, but the module is recorded in the binary's build info and the Trivy image gate reports on module version, not reachability. The rest of the `golang.org/x/*` set moves with it: `mod` `v0.38.0`, `net` `v0.57.0`, `sync` `v0.22.0`, `sys` `v0.47.0`, `tools` `v0.48.0`.
